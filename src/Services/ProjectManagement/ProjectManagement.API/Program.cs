...
// DecVCPlat Project Management logging
Log.Logger = new LoggerConfiguration()
    .WriteTo.Console(outputTemplate: "[{Timestamp:HH:mm:ss} {Level:u3}] DecVCPlat-ProjectMgmt: {Message:lj}{NewLine}{Exception}")
    .WriteTo.File("logs/decvcplat-projectmgmt-.log", rollingInterval: RollingInterval.Day)
    .CreateLogger();

builder.Host.UseSerilog();
builder.Services.AddControllers();

// DecVCPlat Project Management database
builder.Services.AddDbContext<ProjectDbContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("DecVCPlatConnection") ?? 
        "Server=localhost;Database=DecVCPlat_ProjectManagement;Trusted_Connection=true;TrustServerCertificate=true;"));
...
// DecVCPlat authorization policies for project management
...
// DecVCPlat Project Management services
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    {
        Title = "DecVCPlat Project Management API",
        Version = "v1.0",
        Description = "Decentralized Venture Capital Platform - Project Management Microservice"
    });
...
{
    app.UseSwagger();
    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "DecVCPlat Project Management v1"));
}
...
using (var scope = app.Services.CreateScope())
{
    ...
    Log.Information("DecVCPlat Project Management database initialized");
}

Log.Information("DecVCPlat Project Management Service started");
app.Run();
