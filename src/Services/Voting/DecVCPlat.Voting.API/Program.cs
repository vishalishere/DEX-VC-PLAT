...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Description = "API for managing voting in the DecVCPlat platform",
...
// Add Redis cache for voting results
builder.Services.AddStackExchangeRedisCache(options =>
{
    options.Configuration = builder.Configuration.GetConnectionString("RedisCache");
    options.InstanceName = "DecVCPlat_Voting_";
});

// Add SignalR for real-time voting updates
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "DecVCPlat Voting API v1");
        options.RoutePrefix = string.Empty; // Set Swagger UI at the root
        options.DocumentTitle = "DecVCPlat Voting API Documentation";
    });
}
else
{
